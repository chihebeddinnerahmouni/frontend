  ...
  return (
        <div className='flex w-full justify-around items-center bg-white h-[55px] rounded-tl-20 rounded-tr-20 shadow-panelShadow fixed bottom-0 z-40'>
            {navLinks.map((navLink, index) => (
                <NavLink key={index} to={navLink.path} className={(navClass)=> navClass.isActive? 'text-darkGreen4 text-[18px] hover:text-greenHover bg-superLightGreenBackground h-[35px] w-[35px] flex justify-center items-center rounded-50 ' : 'text-darkGreen4 text-[18px] hover:text-greenHover'}>
                    {navLink.icon}
                </NavLink>
            ))}
        </div>
